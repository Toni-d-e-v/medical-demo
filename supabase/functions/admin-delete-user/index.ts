...
serve(async (req) => {
  ...
  try {
    ...
    // Check target user role - admins can only delete users, not other admins
    ...
    if (role === "admin" && targetRole?.role !== "user") {
      throw new Error("Admins können nur User entfernen");
    }
